...
class ImageViewer(QGraphicsView):
    selection_made = pyqtSignal(QPoint)
    selection_removed = pyqtSignal()
    selection_moved = pyqtSignal()
    ...
    def setPhoto(self, pixmap = None):
        ...
        if not self.hasPhoto():
            self._zoom = 0
        self.resetView(self._scale_factor ** self._zoom)
    ...
    # Implementing ZoomNativeGesture for MacOS.
    ...
    def mousePressEvent(self, event):
        if self._mode == 1 and self._image.isUnderMouse():   
            ...
            self.selection_made.emit(point)
        else:
